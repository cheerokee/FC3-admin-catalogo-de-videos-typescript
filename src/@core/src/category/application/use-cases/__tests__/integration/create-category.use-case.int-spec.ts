...
describe("CreateCategoryUseCase Integration Tests",() => {
  ...
  //   ]
  //
  //   test.each(arrange)('from $inputProps to $outputProps',async ({ inputProps, outputProps }) => {
  ...
  it("should create a category", async () => {
    ...
    output = await useCase.execute({ name: 'test',description: 'some description', is_active: true });
    entity = await repository.findById(output.id);
    expect(output).toStrictEqual({
      id: entity.id,
      name: 'test',
