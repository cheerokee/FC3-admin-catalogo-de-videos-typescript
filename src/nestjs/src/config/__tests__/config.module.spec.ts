  ...
  describe('DB Schema', () => {
    ...
    describe('DB_PORT',() => {
      test('invalid cases', () => {
        ...
        expectValidate(schema,{ DB_VENDOR: 'mysql', DB_PORT: '1.2' }).toContain('"DB_PORT" must be an integer')
      });
    ...
    describe('DB_LOGGING',() => {
      test('invalid cases', () => {
        ...
        expectValidate(schema,{ DB_LOGGING: 'a' }).toContain('"DB_LOGGING" must be a boolean')
      });
      ...
      test('valid cases', () => {
        ...
        arrange.forEach((value) => {
          expectValidate(schema, { DB_LOGGING: value }).not.toContain('DB_LOGGING');
        })
      });
    });
...
describe("ConfigModule Unit Tests", () => {
  it('should throw an error when env vars are invalid', () => {
    try {
      ...
      fail('ConfigModule should throw erro when env vars are invalid');
    } catch (e) {
