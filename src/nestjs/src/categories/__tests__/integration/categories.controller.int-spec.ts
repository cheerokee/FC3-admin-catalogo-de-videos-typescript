...
describe('CategoriesController Integration Tests', () => {
  ...
  describe('should create a category', () => {
    ...
    test.each(arrange)('with request $request', async ({ request, expectedPresenter }) => {
  ...
  describe('search method', () => {
    it('should returns categories using query empty ordered by created_at',async () => {
      ...
      for(const item of arrange) {
        ...
        expect(presenter).toEqual(new CategoryCollectionPresenter(item.expected))
      }
    });
    ...
    it('should returns output using paginate, sort and filter',async () => {
      ...
      const arrange_desc: { send_data: any, expected: any }[] = [
        {
          send_data: {
            page: 1,
            per_page: 2,
